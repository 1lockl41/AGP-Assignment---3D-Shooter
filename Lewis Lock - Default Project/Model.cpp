...
void Model::Draw(XMMATRIX *world, XMMATRIX* view, XMMATRIX* projection, XMVECTOR directional_light_colour, XMVECTOR ambient_light_colour, XMVECTOR directional_light_shines_from)
{
	m_pImmediateContext->VSSetShader(m_pVShader, 0, 0);
	m_pImmediateContext->PSSetShader(m_pPShader, 0, 0);
	m_pImmediateContext->IASetInputLayout(m_pInputLayout);
...
bool Model::CheckCollisions(Model* model)
{
	...
	XMVECTOR current_model_position = model->GetBoundingSphereWorldSpacePosition();
	...
	XMVECTOR this_model_position = this->GetBoundingSphereWorldSpacePosition();
	float x2 = XMVectorGetX(this_model_position);
	float y2 = XMVectorGetY(this_model_position);
	float z2 = XMVectorGetZ(this_model_position);

	float distance_squared = pow(x1 - x2, 2) + pow(y1 - y2, 2) + pow(z1 - z2, 2);

	if (distance_squared < pow(model->GetBoundingSphereRadius() + this->GetBoundingSphereRadius(), 2))
	{
		return true;
	}
